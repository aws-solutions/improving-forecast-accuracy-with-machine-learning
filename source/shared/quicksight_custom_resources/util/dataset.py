...
class DataSet(QuickSightResource):
    def __init__(
        ...
        # override these to set schema/ table name directly
        self.schema = None
    ...
    def delete(self):
        ...
        logger.info(
            f"finished deleting quicksight dataset for id:{self.id}, "
            f"response:{response}"
        )
    ...
    def _update_table_name(self, obj):
        for (key, value) in obj.items():
            logger.debug(
                f"Updating table name value of RelationalTable.Name in {key} PhysicalTableMap"
            )
            value["RelationalTable"]["Name"] = self.table_name

    def _update_schema_arn(self, obj):
        for (key, value) in obj.items():
            logger.debug(
                f"Updating schema arn value of RelationalTable.Schema in {key} PhysicalTableMap"
            )
